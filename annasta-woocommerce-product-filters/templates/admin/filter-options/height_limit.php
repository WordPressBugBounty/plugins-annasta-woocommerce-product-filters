<?php if ( ! defined( 'ABSPATH' ) ) { exit; } ?>
              <tr class="awf-hide-for-range-slider awf-hide-for-daterangepicker">
                <td>
                  <label for="<?php echo esc_attr( $filter->prefix ); ?>height_limit"><?php esc_html_e( 'Limit filter height', 'annasta-filters' ); ?></label>
                ...
                <td>
                  <input name="<?php echo esc_attr( $filter->prefix ); ?>height_limit" id="<?php echo esc_attr( $filter->prefix ); ?>height_limit" type="text" value="<?php echo esc_attr( $value ); ?>" style="width: 5em;">
                  <span style="padding-right:2em;"><?php esc_html_e( 'pixels', 'annasta-filters' ); ?></span>

                  <label for="<?php echo esc_attr( $filter->prefix ); ?>height_limit_style" class="awf-secondary-label"><?php esc_html_e( 'Limitation style', 'annasta-filters' ); ?></label>
...
?>
                  ...
                  <label for="<?php echo esc_attr( $filter->prefix ); ?>shrink_height_limit" class="awf-secondary-label" style="display:inline-flex;margin: 5px 0;align-items: center;white-space:nowrap;">
                    <input type="checkbox" style="margin-right:-5px;" name="<?php echo esc_attr( $filter->prefix ); ?>shrink_height_limit" id="<?php echo esc_attr( $filter->prefix ); ?>shrink_height_limit" value="yes"<?php if( ! empty( $filter->settings['shrink_height_limit'] ) ) { echo ' checked="checked"'; } ?>>
