<?php if( ! defined( 'ABSPATH' ) ) { exit; } ?>
              <tr class="awf-hide-for-range-slider awf-hide-for-daterangepicker">
                <td>
                  <label for="<?php echo esc_attr( $filter->prefix ); ?>show_search"><?php esc_html_e( 'Display items search', 'annasta-filters' ); ?></label>
                ...
                <td>
                  <div class="awf-fo-flex1">
                    <input type="checkbox" name="<?php echo esc_attr( $filter->prefix ); ?>show_search" id="<?php echo esc_attr( $filter->prefix ); ?>show_search" value="yes"<?php if( ! empty( $value ) ) { echo ' checked="checked"'; } ?>>
